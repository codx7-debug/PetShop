...
export default function OrgClinicPetDocsScreen() {
  const { t } = useLanguage();
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/org/clinic/pets/${pid}/documents`, {
      ...
      Alert.alert("", t("orgClinicPetDocs.loadError"));
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const save = async () => {
    if (!title.trim() || !fileUrl.trim()) {
      Alert.alert("", t("orgClinicPetDocs.validation"));
      return;
    }
    setSaving(true);
    try {
      ...
      Alert.alert("", t("orgClinicPetDocs.saveError"));
    } finally {
      setSaving(false);
    }
  };
  ...
  if (!Number.isFinite(pid)) {
    return (
      <SafeAreaView style={styles.safe} edges={["top"]}>
        <Text style={styles.bad}>{t("orgClinicPetDocs.missingPet")}</Text>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <View style={styles.header}>
        ...
        <Text style={styles.title} numberOfLines={1}>
          {t("orgClinicPetDocs.docs")} · {petName || `${t("orgClinicPetDocs.pet")} ${pid}`}
        </Text>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.empty}>{t("orgClinicPetDocs.empty")}</Text>}
          renderItem={({ item }) => (
      ...
      <OrgKeyboardModalSheet visible={open} onRequestClose={() => setOpen(false)}>
        <View style={styles.sheet}>
          <Text style={styles.sheetTitle}>{t("orgClinicPetDocs.addDocumentLink")}</Text>
          <TextInput placeholder={t("orgClinicPetDocs.titlePh")} style={styles.inp} value={title} onChangeText={setTitle} />
          <TextInput
            placeholder={t("orgClinicPetDocs.fileUrlPh")}
            style={styles.inp}
            value={fileUrl}
            ...
            autoCorrect={false}
          />
          <TextInput placeholder={t("orgClinicPetDocs.notesPh")} style={styles.inp} value={notes} onChangeText={setNotes} />
          <View style={styles.sheetRow}>
            ...
            <TouchableOpacity style={styles.go} disabled={saving} onPress={() => void save()}>
              {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.goTxt}>{t("orgClinicPetDocs.save")}</Text>}
            </TouchableOpacity>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  sheetTitle: { fontSize: 17, fontWeight: "800", marginBottom: 12, color: "#0f172a" },
  inp: {
    borderWidth: 1,
    borderColor: "#e2e8f0",
    borderRadius: 12,
